...
func TestSSOAppRoleUpgradeDoesNotPreserveGlobalAdmin(t *testing.T) {
	...
	for _, name := range []string{"linked", "local"} {
		...
		if name == "linked" && (role != "user" || revoked == "" || cap != 0) {
	...
	if err := db.QueryRow(`SELECT object_id,reason_code FROM audit_events WHERE event='auth.sso_role_upgrade'`).Scan(&subject, &reason); err != nil || subject != "subject" || reason != "legacy_role=admin" {
